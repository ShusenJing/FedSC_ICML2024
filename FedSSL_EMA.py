...
def main(parser):
    ...
    for i in range(client_num):
        client_tmp = Client_EMA(dataset=train_dataset_split[i], model=global_model, batch_size=batch_size, local_epoch=local_epoch, 
                                comm_rounds=comm_rounds, beta=0.99, device=device, gd_clip=args_dict['gd_clip'], weights_scalar = args_dict['weights_scalar'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/ema.yaml',
