...
from models import ConcatModel, resnet18, resnet18_noBN, resnet50
...
def main(parser):
    ...
    for i in range(client_num):
        client_tmp = Client_SPECshare(dataset=train_dataset_split[i], model=global_model, batch_size=batch_size, 
                                    local_epoch=local_epoch, comm_rounds=comm_rounds, 
                                    device=device, mu = mu)
    ...
    for com in range(comm_rounds):
        ...
        alpha = alpha_max - (alpha_max-alpha_min)*com/comm_rounds if args_dict['share_cov'] else 1
        if args_dict['client_participation'] == 'full' or com == 0:
        ...
        for j in client_selected:
            ...
            COV_list[j]=client_list[j].compCOV(cov_noise = 0.0052/10*2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/spec.yaml',
