...
class FixupBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_tanh = False):
...
class FixupResNet(nn.Module):
    ...
    def __init__(self, block, layers, widen_factor=8, num_classes=10, is_tanh = False):
        super(FixupResNet, self).__init__()
        self.num_layers = sum(layers)
        self.widen_factor = widen_factor
        self.inplanes = 16*self.widen_factor
        self.conv1 = conv3x3(3, 16*self.widen_factor)
        ...
        if self.is_tanh:
            ...
            self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16*self.widen_factor, layers[0])
        self.layer2 = self._make_layer(block, 32*self.widen_factor, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 64*self.widen_factor, layers[2], stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.bias2 = nn.Parameter(torch.zeros(1))
        self.fc = nn.Linear(64*self.widen_factor, num_classes)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.is_tanh))
