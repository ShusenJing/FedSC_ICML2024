...
from clients import Client_SPECshare, Client_SPEC, Client_EMA,Client_BYOL,Client_X
...
def main(parser):
    ...
    for i in range(client_num):
        client_tmp = Client_X(dataset=train_dataset_split[i], model=global_model, batch_size=batch_size, local_epoch=local_epoch, comm_rounds=comm_rounds, device=device)
    ...
    for com in range(comm_rounds):
        ...
        for j in range(len(client_list)):
            ...
            client_list[j].train(global_model, lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/x.yaml',
